...
Suggested Prompt: None provided, will perform self-exploration around 2 questions: What countries have the lowest Voice and Accountability ratings today? What countries had the greatest drop in Voice and Accountability ratings since they began tracking for that country?
...
```{r}
data <- read_csv("free_speech_indicators.csv")
```
...
```{r}
countries_data$growth_ratio <- countries_data$value %>% lapply(get_total_change) %>% unlist()
```

View the distribution of newly created growth_ratio column

```{r}
countries_data %>%
  ggplot() +
  geom_boxplot(aes(x=growth_ratio))
```
